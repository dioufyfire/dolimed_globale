    ...
    function list_of_documents($filearray,$object,$modulepart,$param,$forcedownload=0,$relativepath='',$permtodelete=1,$useinecm=0,$textifempty='',$maxlength=0)
    {
        ...
        foreach($filearray as $key => $file)      // filearray must be only files here
        {
            ...
            && ! preg_match('/\.meta$/i',$file['name']))
            {
                ...
                print '</a>';
				$a = $relativepath.$file['name'];
				print '<a class="pictopreview documentpreview" href="/dolibarr/document.php?modulepart='.$modulepart;
				if (strpos($a, 'jpeg') == true | strpos($a, 'jpg') == true) {
					print '&attachment=0&amp;file='.urlencode($relativepath.$file['name']).'" mime="image/jpeg" target="_blank">';
				} else {
				print '&attachment=0&amp;file='.urlencode($relativepath.$file['name']).'" mime="application/pdf" target="_blank">';
				}
				print '<span class="fa fa-search-plus" style="color: gray">';
				print '</span></a>';
                print "</td>\n";
