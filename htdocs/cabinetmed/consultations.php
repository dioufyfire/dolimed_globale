...
$mode_cons = GETPOST('mode_cons', 'alpha');
$posted_specialite = GETPOST('options_specialite', 'alpha');

if ($mode_cons != '2' && $posted_specialite == '2') {
    $mode_cons = '2';
}
...
$activeconsulttab = ($mode_cons == '2') ? 'tabconsultationsped' : 'tabconsultations';

$is_pediatrie = ($mode_cons == '2');
$is_medecine_generale = ($mode_cons == '1');

$extrafields_systeme = array(
    'specialite'
);

$extrafields_pediatrie = array(
    'poids',
    'taille_cm',
    'temperature',
    'perimetre_cranien',
...
if ($action == 'add' || $action == 'update')
{
    if (! GETPOST('cancel','alpha'))
    {
        ...
        {
            if ($mode_cons == '2') {
                $_POST['options_specialite'] = '2';
            } elseif (GETPOST('options_specialite', 'alpha') == '2') {
                $_POST['options_specialite'] = '2';
                $mode_cons = '2';
            } else {
                $_POST['options_specialite'] = '1';
            }
        ...
        {
            $db->commit();
            header("Location: ".$_SERVER["PHP_SELF"].'?socid='.$consult->fk_soc.'&mode_cons='.$mode_cons);
            exit(0);
        }
        else
        {
...
/// CUSTOM MOMO
$hide_options = $extrafields_systeme;

if (!$is_pediatrie) {
    $hide_options = array_merge($hide_options, $extrafields_pediatrie);
}

print '<script type="text/javascript">'."\n";
print 'jQuery(function() {'."\n";

foreach ($hide_options as $field) {
    print '  jQuery("[name=\'options_'.$field.'\']").closest("tr").hide();'."\n";
}
...
else
{
	...
	// General
	print '<form method="post" action="'.$_SERVER["PHP_SELF"].'?socid='.$socid.'&mode_cons='.$mode_cons.'">';
	...
	print '<input type="hidden" name="token" value="'.$_SESSION['newtoken'].'">';
        ...
        // CUSTOM
	if ((float) DOL_VERSION < 7) dol_fiche_head($head, $activeconsulttab, $langs->trans("Patient"), 0, 'patient@cabinetmed');
	else dol_fiche_head($head, $activeconsulttab, $langs->trans("Patient"), -1, 'patient@cabinetmed');
...
//        print '<legend>'.$langs->trans("Diagnostiques et prescriptions").'</legend>'."\n";
        ...
        // Payment area
        print '<table class="notopnoleftnoright" id="paymentsbox" width="100%"style="display:none"style="display:none" >';
        ...
        print '<input type="text" class="flat" name="montant_tiers" id="idmontant_tiers" value="'.($consult->montant_tiers!='0'?price($consult->montant_tiers):'0').'" size="5">';
...
if ($action == '' || $action == 'delete')
{
    ...
    {
        print '<a class="butAction" href="'.$_SERVER["PHP_SELF"].'?socid='.$object->id.'&amp;action=create&mode_cons='.$mode_cons.'">'.$langs->trans("NewConsult").'</a>';
    }
...
//    $sql.= " LEFT JOIN ".MAIN_DB_PREFIX."bank as b on bu.fk_bank = b.rowid";
    ...
    if ($mode_cons == '2') {
        $sql .= " AND ef.specialite = '2'";
    } else {
        $sql .= " AND (ef.specialite IS NULL OR ef.specialite = '' OR ef.specialite = '1')";
    }
    ...
    if ($resql)
    {
        ...
        while ($i < $num)
        {
            ...
            print '<td>';
            //print '<a href="'.$_SERVER["PHP_SELF"].'?socid='.$obj->fk_soc.'&id='.$obj->rowid.'&action=edit&mode_cons='.$mode_cons">'.sprintf("%08d",$obj->rowid).'</a>';
            print '<a href="'.$_SERVER["PHP_SELF"].'?socid='.$obj->fk_soc.'&id='.$obj->rowid.'&action=edit&mode_cons='.$mode_cons.'">'.sprintf("%08d",$obj->rowid).'</a>';
            print '</td>';
            print '<td>';
            ...
            print '<a href="'.$_SERVER["PHP_SELF"].'?socid='.$obj->fk_soc.'&id='.$obj->rowid.'&action=edit&mode_cons='.$mode_cons.'">'.img_edit().'</a>';
